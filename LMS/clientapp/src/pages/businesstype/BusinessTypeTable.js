...
import { setSelectedBusiness, fetchBusinessType } from "./../../store/reducers/masters";
...
export default function BusinessTypeTable() {
    ...
    const dispatch = useDispatch();
    const { businessTypeData } = useSelector((state) => state.masters);
    ...
    const onEditClick = (row) => {
        dispatch(setSelectedBusiness({ businessTypeDetails: row }));
        dispatch(openDrawer({ drawerOpen: true }));
    };
    ...
    const handleSearch = (e) => {
        const filtered = businessTypeData.filter(
            (item) =>
                item.BusinessTypeName.toLowerCase().indexOf(e.target.value.toLowerCase()) > -1 
    ...
    useEffect(() => {
        setFilteredData(businessTypeData);
    }, [businessTypeData]);
